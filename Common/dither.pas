unit dither; 
      ...
      SBitMap.PixelFormat := pf24bit;   //24
      DBitMap.Width := SBitMap.Width; 
      DBitMap.Height := SBitMap.Height; 

      GetMem(pal, sizeof(TLogPalette) + sizeof(TPaletteEntry) * 255); 
      BuildColorTable(ColorCount, Pal); 
      DBitMap.Palette := BuildColorTable(ColorCount, Pal); // Set 
     // DBitMap.Palette
      FreeMem(pal); 

      for i := 0 to (DBitMap.Height - 1) do 
      begin 
      SPtr := SBitMap.ScanLine[i]; 
      DPtr := DBitMap.ScanLine[i]; 
      for j := 0 to (DBitMap.Width - 1) do 
      begin
